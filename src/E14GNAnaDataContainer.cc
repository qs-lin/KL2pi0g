...
void E14GNAnaDataContainer::branchOfClass(TTree *tree,int mode){
  ...
  if(m_mode>=4){
	...
	tree->Branch("Klong_gamNumber",&Klong_gamNumber,"Klong_gamNumber/I");  
	tree->Branch("Klong_gamIndex",Klong_gamIndex,"Klong_gamIndex[KlongNumber][100]/I");  
...
void E14GNAnaDataContainer::setBranchAddress(TTree *tree){
  ...
  if(tree->GetBranch("KlongNumber")!=0){
    ...
    tree->SetBranchAddress("Klong_gamIndex",Klong_gamIndex);

    tree->SetBranchAddress("KlongCoe",KlongCoe);
    tree->SetBranchAddress("KlongTime",KlongTime);
    tree->SetBranchAddress("KlongCsiTime",KlongCsiTime);
  }
  if(tree->GetBranch("CsiNumber")!=0){
...
bool E14GNAnaDataContainer::getData(std::vector<KL2pi0g> &klist) const{
  //if( 2*KlongNumber != Pi0Number && 3*KlongNumber != Pi0Number ){
  if( 2*KlongNumber != Pi0Number && 5*KlongNumber  != GammaNumber){ 
    GsimMessage::getInstance()
      ->report("error",Form("KlongNumber is inconsistent with Pi0Number"));
    return false;
  }
...
bool E14GNAnaDataContainer::getData(KL2pi0g &kl,int ID) const{
  if(ID<0||ID>=KlongNumber){
    GsimMessage::getInstance()
      ->report("warning",Form("%d th Klong are not exist",ID));
    return false;
  ...
  kl.SetVtx(KlongPos[ID][0],KlongPos[ID][1],KlongPos[ID][2]);
  kl.SetP3(KlongMom[ID][0],KlongMom[ID][1],KlongMom[ID][2]);
  kl.SetCoe(KlongCoe[ID][0],KlongCoe[ID][1],KlongCoe[ID][2]);
  kl.SetDeltaZ(KlongDeltaZ[ID]);
  kl.SetChisqZ(KlongChisqZ[ID]);
  kl.SetSortFlag(KlongSortFlag[ID]);
  //kl.setVertexFlag(KlongVertFlag[ID]);

  kl.SetTime(KlongTime[ID]);
  kl.SetCsiTime(KlongCsiTime[ID]);
  ...
  Gamma& g5 = kl.g5(); 
  if(!getGamma(g5,Klong_gamIndex[ID][0]))
    return false;
...
void  E14GNAnaDataContainer::setData( std::vector<KL2pi0g>const &klVec ){
  static bool warning = true;
  if(warning && (m_mode & 0x4) == 0 ){
    GsimMessage::getInstance()
      ->report("warning","E14GNAnaDataContainer::setData(std::list<Klong>) was called but branchOfKlong() was not called. This data will not be filled to TTree.");
...
void  E14GNAnaDataContainer::setData( Klong const &kl ,bool standalone ){
  ...
  //KlongTime[id]    = kl.t();
  //KlongCsiTime[id] = kl.csi_t();
  for(int i=0;i<3;i++){
    KlongMom[id][i]=kl.p3()(i);
    KlongPos[id][i]=kl.v()(i);
    //KlongCoe[id][i]=kl.v()(i);
  }
  for(int i=0;i<3;i++){
    Klong_piIndex[id][i] = -1;
  }
  for(unsigned int i=0;i<kl.pi0().size();i++){
    Klong_piIndex[id][i] = Pi0Number;
    setPi0(kl.pi0()[i]);
  }

  int indg = 0;
  for(std::list<Gamma>::const_iterator it = kl.gamma().begin(); it!=kl.gamma().end(); it++){
    if(indg>=100)continue;
	Klong_gamIndex[id][indg] = GammaNumber;
	indg++;
    setGamma(*it);
  }
  Klong_gamNumber = indg;
  KlongNumber++;

  if(standalone)  setGamClus();
}

void  E14GNAnaDataContainer::setData( KL2pi0g const &kl ,bool standalone ){
  static bool warning = true;
  if(standalone){
    if(warning && (m_mode & 0x4) == 0 ){
      GsimMessage::getInstance()
